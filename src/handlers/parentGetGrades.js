...
function formatStudentGrades (studentCourses, courseSlot) {
  return studentCourses.map(({ student, courses }) => `For ${student}: ${formatCourses(courses, courseSlot)}`).join('\n')
}
...
function formatCourse (course) {
  ...
  const response = grade ? `In ${course.name}: the grade is ${grade}.` : `No grade posted for ${course.name}.`
  return response
}
