...
module.exports = {
  GetCalendarEvents: function () {
    ...
    this.context.api.getActiveStudentCourses()
      .then(res => {
        const matchingCourses = courseSlot ? parseCourseSlot(res.data, courseSlot).slice(0, 1) : res.data
      ...
      .then(eventsByDate => {
        ...
        const dates = Object.keys(eventsByDate).map(date => ({
          date, label: formatDate.forSpeech(new Date(date)),
        }))
        ...
        if (dates.length) {
          speechResponse = `Here are your events for ${formatDate.rangeForSpeech(startDate, endDate)}${courseSuffix}: `
          ...
          speechResponse = `You have no events for ${formatDate.rangeForSpeech(startDate, endDate)}${courseSuffix}. `
        }
