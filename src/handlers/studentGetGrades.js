...
module.exports = {
  'StudentGetGrades': function () {
    this.context.api.getActiveStudentCourses(['total_scores'])
      .then((res) => {
        let speechResponse = ''
...
function formatGrades (courses, courseSlot) {
  ...
  if (!results.length) {
    ...
    if (courseSlot && results.length === 1) {
      ...
      speechResponse = `For ${course.name} your grade is ${enrollment.computed_current_grade || enrollment.computed_current_score || 'not posted yet'} .`
    } else {
      speechResponse = 'Here are your grades:\n'
      speechResponse += results.map(course => {
        const enrollment = course.enrollments[0] || {}
        return `for ${course.name}: Your grade is ${enrollment.computed_current_grade || enrollment.computed_current_score || 'not posted yet'} .`
      }).join('\n')
    }
  }
